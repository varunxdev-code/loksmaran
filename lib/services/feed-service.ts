...
import { freesoundClip } from "./freesound";
import { cached } from "./http";
import { unpackId } from "./ids";
import { getOsmPlace, searchOsm } from "./osm";
...
const TOPIC: Record<FeedCategory, string> = {
  all: "heritage culture tradition",
  nearby: "heritage temple village",
  villages: "village",
  cities: "city",
  heritage: "heritage monument temple fort",
  festivals: "festival",
  food: "cuisine food dish",
  crafts: "handicraft craft textile",
  music: "folk music song",
  stories: "folklore legend story",
  history: "history fort archaeology",
  traditions: "tradition ritual custom",
...
async function fillImages(items: FeedItem[]) {
  const missing = items.filter((i) => !i.image).slice(0, 8);
  await Promise.all(
    missing.map(async (item) => {
      item.image = await commonsImageFor(`${item.title} ${item.state || "India"}`);
    }),
  );
...
async function maybeAudio(items: FeedItem[], category: FeedCategory, q: string) {
  if (!["music", "festivals", "traditions", "stories", "folk"].includes(category) && category !== "music") return items;
  const wants = category === "music" || category === "festivals" || category === "traditions" || category === "stories";
  if (!wants) return items;
  const clip = (await archiveAudio(q)) || (await freesoundClip(q));
  if (!clip?.url) return items;
  let attached = 0;
  return items.map((item) => {
    if (item.audio || attached >= 3) return item;
    attached += 1;
    return {
      ...item,
      audio: clip.url,
      license: item.license ? `${item.license}; audio: ${clip.license}` : clip.license,
    };
  });
}
...
async function assemble(opts: FeedQuery): Promise<FeedItem[]> {
  ...
  const secondary =
    page === 1
      ? await Promise.allSettled([
          searchUnsplash(q, category, opts.state),
          searchPexels(q, category, opts.state),
          category === "heritage" || category === "history" ? searchEuropeana(q, category, opts.state) : Promise.resolve([]),
        ])
      : [];
  ...
  const enriched = await Promise.all(items.slice(0, 14).map((item) => enrichWithWikipedia(item)));
  items = dedupe([...enriched, ...items.slice(14)]);
...
export async function getFeed(opts: FeedQuery) {
  const page = Math.max(1, opts.page || 1);
  const cacheKey = `feed:${JSON.stringify(opts)}`;
  const items = await cached(cacheKey, 8 * 60_000, () => assemble(opts));
  const start = 0;
  const slice = items.slice(start, PAGE);
  return {
    items: slice,
    page,
    pageSize: PAGE,
    hasMore: items.length > PAGE || page < 8,
  };
}
...
export async function getLocations(level: "state" | "district" | "place", state?: string, district?: string) {
  ...
  if (level === "district" && state) {
    ...
    return rows.map((r) => ({ name: r.name, qid: r.qid }));
  }
  if (level === "place" && state) {
    const rows = await listPlaces(state, district);
    return rows.map((r) => ({ name: r.name, qid: r.qid, district: r.district, coordinates: r.coordinates }));
  }
  return [];
}

export async function trendingTopics() {
  const items = await getFeed({ category: "festivals", page: 1 });
  const names = items.items.map((i) => i.title).slice(0, 8);
  return names.length ? names : ["Holi", "Pongal", "Onam", "Bihu", "Pattachitra", "Ikat"];
}
